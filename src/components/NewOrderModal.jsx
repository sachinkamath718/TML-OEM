...
export default function NewOrderModal({ onClose, onCreate }) {
  ...
  const sectionStyle = {
    borderTop: '1px solid #F3F4F6', paddingTop: 14, marginBottom: 14,
  };
  const sectionTitle = {
  ...
  function updateProductDuration(vehicleId, productName, duration) {
    setVehicles((prev) => prev.map((v) => {
      ...
      const products = v.products.map((p) =>
        p.name === productName ? { ...p, duration_in_years: Number(duration) } : p
      );
      return { ...v, products };
    }));
  }
  ...
  async function handleCreate() {
    ...
    const historyEntry = {
      ...
      user:      { name: spocName, phone: spocPhone, role: 'SPOC' },
      note:      `Bulk order created with ${vehicles.length} vehicle(s)`,
    };

    const supabaseOrders = vehicles.flatMap((v) =>
      MODULES.map((module) => ({
    ...
    onCreate(tmlPayload, supabaseOrders);
    setLoading(false);
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={sectionStyle}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
            ...
            <div>
              ...
              <input value={custPan} onChange={(e) => setCustPan(e.target.value)} placeholder="AAFCS7862Q" style={inputStyle} />
            </div>
            <div>
              <label style={labelStyle}>GST</label>
              <input value={custGst} onChange={(e) => setCustGst(e.target.value)} placeholder="29AAFCS7862Q1ZK" style={inputStyle} />
            </div>
            <div>
              <label style={labelStyle}>Contact Number *</label>
              <input value={custContact} onChange={(e) => setCustContact(e.target.value)} placeholder="+918800441122" style={inputStyle} />
            </div>
            <div style={{ gridColumn: '2 / -1' }}>
              <label style={labelStyle}>Email</label>
              <input value={custEmail} onChange={(e) => setCustEmail(e.target.value)} placeholder="ops@company.in" style={inputStyle} />
            </div>
          </div>
        </div>
        ...
        <div style={sectionStyle}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
            <div style={{ gridColumn: '1 / -1' }}>
              ...
              <input value={locAddress} onChange={(e) => setLocAddress(e.target.value)} placeholder="Survey No. 45, Bhosari Industrial Estate" style={inputStyle} />
            </div>
            <div>
              <label style={labelStyle}>City *</label>
              <input value={locCity} onChange={(e) => setLocCity(e.target.value)} placeholder="Pune" style={inputStyle} />
            </div>
            <div>
              <label style={labelStyle}>District</label>
              <input value={locDistrict} onChange={(e) => setLocDistrict(e.target.value)} placeholder="Pune" style={inputStyle} />
            </div>
            <div>
              <label style={labelStyle}>Pincode</label>
              <input value={locPincode} onChange={(e) => setLocPincode(e.target.value)} placeholder="411026" style={inputStyle} />
            </div>
            <div style={{ gridColumn: '3 / -1' }}>
              <label style={labelStyle}>State *</label>
              <input value={locState} onChange={(e) => setLocState(e.target.value)} placeholder="Maharashtra" style={inputStyle} />
            </div>
          </div>
        </div>
        ...
        <div style={sectionStyle}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
            <div>
              ...
              <input value={spocName} onChange={(e) => setSpocName(e.target.value)} placeholder="Suresh Patil" style={inputStyle} />
            </div>
            <div>
              <label style={labelStyle}>Phone</label>
              <input value={spocPhone} onChange={(e) => setSpocPhone(e.target.value)} placeholder="+917709988001" style={inputStyle} />
            </div>
            <div>
              <label style={labelStyle}>Email</label>
              <input value={spocEmail} onChange={(e) => setSpocEmail(e.target.value)} placeholder="spoc@company.in" style={inputStyle} />
            </div>
          </div>
        </div>
            ...
            {vehicles.map((v, idx) => (
              <div key={v.id} style={{ background: '#F9FAFB', borderRadius: 10, padding: '14px', border: '1px solid #E5E7EB' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                  ...
                  {vehicles.length > 1 && (
                    <button onClick={() => removeVehicle(v.id)} style={{ background: '#FEF2F2', border: '1px solid #FECACA', color: '#DC2626', borderRadius: 6, padding: '3px 10px', fontSize: 11, fontWeight: 600, cursor: 'pointer' }}>
                      Remove
                    </button>
                  )}
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 8 }}>
                  <div>
                    ...
                    <input value={v.vin} onChange={(e) => updateVehicle(v.id, 'vin', e.target.value)} placeholder="17-char VIN" style={inputStyle} />
                  </div>
                  <div>
                    <label style={labelStyle}>Reg. Number</label>
                    <input value={v.registration_no} onChange={(e) => updateVehicle(v.id, 'registration_no', e.target.value)} placeholder="MH12DE4570" style={inputStyle} />
                  </div>
                  <div>
                    <label style={labelStyle}>Engine No *</label>
                    <input value={v.engine_no} onChange={(e) => updateVehicle(v.id, 'engine_no', e.target.value)} placeholder="CUMMINS6BT5300" style={inputStyle} />
                  </div>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 8 }}>
                  <div>
                    ...
                    <input value={v.make} onChange={(e) => updateVehicle(v.id, 'make', e.target.value)} placeholder="TATA" style={inputStyle} />
                  </div>
                  <div>
                    <label style={labelStyle}>Model</label>
                    <input value={v.model} onChange={(e) => updateVehicle(v.id, 'model', e.target.value)} placeholder="Prima 4028.S" style={inputStyle} />
                  </div>
                  <div>
                    <label style={labelStyle}>Variant</label>
                    <input value={v.variant} onChange={(e) => updateVehicle(v.id, 'variant', e.target.value)} placeholder="4028.S HE" style={inputStyle} />
                  </div>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 8, marginBottom: 8 }}>
                  ...
                  <div>
                    <label style={labelStyle}>RTO State *</label>
                    <input value={v.rto_state} onChange={(e) => updateVehicle(v.id, 'rto_state', e.target.value)} placeholder="MH" style={inputStyle} />
                  </div>
                </div>

                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 10 }}>
                  <div>
                    <label style={labelStyle}>RTO Office Code *</label>
                    <input value={v.rto_office_code} onChange={(e) => updateVehicle(v.id, 'rto_office_code', e.target.value)} placeholder="MH12" style={inputStyle} />
                  </div>
                </div>
                        ...
                        <div key={p} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                          ...
                          {selected && ['AIS140', 'MINING'].includes(p) && (
                            <input
                              ...
                              value={selected.duration_in_years}
                              onChange={(e) => updateProductDuration(v.id, p, e.target.value)}
                              style={{ ...inputStyle, width: 48, padding: '4px 6px', fontSize: 11 }}
        ...
        {error && (
          <div style={{ fontSize: 12, color: '#DC2626', marginBottom: 12, background: '#FEF2F2', padding: '8px 12px', borderRadius: 6 }}>{error}</div>
        )}
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          ...
          <button
            ...
            disabled={loading}
            style={{ padding: '9px 22px', borderRadius: 8, border: 'none', background: loading ? '#A5B4FC' : '#4F46E5', color: '#fff', fontSize: 13, fontWeight: 600, cursor: loading ? 'not-allowed' : 'pointer' }}
          >
